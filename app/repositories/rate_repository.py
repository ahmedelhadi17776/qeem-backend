...
class RateRepository:
    ...
    def get_by_user_id(self, user_id: int, skip: int = 0, limit: int = 100) -> List[RateCalculation]:
        """Get rate calculations for a specific user with pagination."""
        stmt = (select(RateCalculation)
    ...
    def update(self, calculation: RateCalculation, calculation_data: dict) -> RateCalculation:
    ...
    def get_favorites(self, user_id: int) -> List[RateCalculation]:
        ...
        stmt = (select(RateCalculation)
                .where(RateCalculation.user_id == user_id, RateCalculation.is_favorite == True)
                .order_by(desc(RateCalculation.created_at)))
        return list(self.db.execute(stmt).scalars().all())
    
    def set_favorite(self, calculation_id: int, user_id: int, is_favorite: bool) -> Optional[RateCalculation]:
    ...
    def get_by_project_type(self, project_type: str, skip: int = 0, limit: int = 100) -> List[RateCalculation]:
